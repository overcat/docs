...
如果事务中的任何操作失败，则所有操作都会失败。例如，假设你有 100 Lumens，你做了两次支付操作，每次支付 60 Lumens。如果您进行两个事务(每个事务有一个操作) ，第一个事务将成功，第二个事务将失败，因为您没有足够的流明。你会剩下 40 Lumens。然而，如果你将这两笔支付放在一个事务中，它们都会失败，并且你的账户中还会剩下 100 Lumens。
